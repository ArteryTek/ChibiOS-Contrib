...
/**
 * @file    AT32F402xx/hal_lld.c
 * @brief   AT32F402xx HAL subsystem low level driver source.
 *
 * @addtogroup HAL
 * @{
 */
 ...
 * @note    It is declared in system_at32f402xx.h.
 */
...
/**
  ...
  * @retval none
  */
static void reduce_power_consumption(void)
{
  ...
  else if(CRM->CTRL & CRM_CTRL_PLLSTBL)
  {
    ...
    while((CRM->CTRL & CRM_CTRL_PLLUSTBL) == 0);
...
void at32_clock_init(void) {
  ...
  CRM->APB2LPEN |= CRM_APB2LPEN_SCFGLPEN;
   ...
   /*
    OTGHS PHY not initialized, resulting in high power consumption
    Solutions:
